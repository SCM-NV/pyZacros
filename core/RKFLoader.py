...
class RKFLoader:
    ...
    def __deriveLatticeAndMechanism(self, results):
        ...
        latticeVectors = [
            [latticeVectors[3 * i + j] / angs for j in range(nLatticeVectors)] for i in range(nLatticeVectors)
        ]
        ...
        if type(products) != list:
            products = [products]
        if type(prefactorsFromReactant) != list:
            prefactorsFromReactant = [prefactorsFromReactant]
        if type(prefactorsFromProduct) != list:
            prefactorsFromProduct = [prefactorsFromProduct]
        ...
        if "nFragments" in rkf_skeleton["EnergyLandscape"]:
            ...
            fragmentsFileNames = (
                results.readrkf("EnergyLandscape", "fragmentsFileNames").replace(".rkf", "").split("\0")
            )
        ...
        if "nFStates" in rkf_skeleton["EnergyLandscape"]:
            ...
            for i in range(nFStates):
                ...
                fStatesAdsorptionPrefactors[i] = results.readrkf(
                    "EnergyLandscape", "fStatesAdsorptionPrefactors(" + str(i + 1) + ")"
                )
                fStatesDesorptionPrefactors[i] = results.readrkf(
                    "EnergyLandscape", "fStatesDesorptionPrefactors(" + str(i + 1) + ")"
                )

                if type(fStatesComposition[i]) != list:
                    fStatesComposition[i] = [fStatesComposition[i]]
                if type(fStatesConnections[i]) != list:
                    fStatesConnections[i] = [fStatesConnections[i]]
                if type(fStatesAdsorptionPrefactors[i]) != list:
                    fStatesAdsorptionPrefactors[i] = [fStatesAdsorptionPrefactors[i]]
                if type(fStatesDesorptionPrefactors[i]) != list:
                    fStatesDesorptionPrefactors[i] = [fStatesDesorptionPrefactors[i]]
        ...
        if (
            "energyReferenceLabels" in rkf_skeleton["EnergyLandscape"]
        ...
        if type(energyReferenceValues) != list:
            energyReferenceValues = [energyReferenceValues]
        ...
        if referenceRegion != results.readrkf("BindingSites", "ReferenceRegionLabel").strip():
            msg = "\n### ERROR ### RKFLoader.__deriveLatticeAndMechanism.\n"
            msg += (
                "              The reference region from the EnergyLandscape is different than for the BindingSites\n"
            )
            raise NameError(msg)
        ...
        latticeDisplacements = [
            [latticeDisplacements[nLatticeVectors * i + j] for j in range(nLatticeVectors)] for i in range(nConnections)
        ]
        ...
        if type(fromSites) != list:
            fromSites = [fromSites]
        if type(toSites) != list:
            toSites = [toSites]
        if type(nParentStates) != list:
            nParentStates = [nParentStates]
        if type(parentStatesRaw) != list:
            parentStatesRaw = [parentStatesRaw]
        if type(parentAtomsRaw) != list:
            parentAtomsRaw = [parentAtomsRaw]
        ...
        def getLatticeRxnSubgraph(bs_from, bs_to):
            ...
            for bs1 in bs_from:
                if not bs1 in latticeSPaths:
                    continue
                for bs2 in bs_to:
                    if not bs2 in latticeSPaths[bs1]:
                        continue
                ...
                for sMol in adsorbateMols:
                    for atom in sMol:
                        ...
                        if all(abs(r1 - r2) < 1e-6):
                            attachedMoleculeData[idState][idSite] = {
                                "formula": sMol.get_formula(),
        ...
        def getProperties(idState):
                ...
                if bs1 in attachedMoleculeData[idState].keys():
                    ...
                    for j, bs2 in enumerate(G1_nodes):
                        ...
                        if bs2 in attachedMoleculeData[idState].keys():
                            ...
                            if speciesName1 == speciesName2 and numpy.linalg.norm(centerOfMass1 - centerOfMass2) < 0.5:
            ...
            if entityNumber.count(-1) != len(G1_nodes):
                denticity = [
                    entityNumber.count(entityNumber[i]) if entityNumber[i] != -1 else 1
        ...
        def getPropertiesForCluster(species, entityNumber):
            ...
            if len(nonEmptySites) > 1:
                ...
                data["entity_number"] = [
                    data["entity_number"][i] - min(data["entity_number"]) for i in range(len(data["entity_number"]))
                ]
        ...
        def getMultiplicity(data):
            ...
            if (
                len(data["site_types"]) == 2
                and data["site_types"][0] == data["site_types"][1]
                and data["species"][0].symbol == data["species"][1].symbol
                and len(data["neighboring"]) == 1
                and data["neighboring"][0] in [[0, 1], [1, 0]]
            ):
                mult = 2
        ...
        for idState in range(nStates):
            if isTS[idState]:
                ...
                # --------------------------------------------------------------------
                # Reaction
                entityNumberReactant = (
                    entityNumberReactant if entityNumberReactant.count(-1) != len(entityNumberReactant) else None
                )
                entityNumberProduct = (
                    entityNumberProduct if entityNumberProduct.count(-1) != len(entityNumberProduct) else None
                )
            ...
            for pos, idState in enumerate(fStatesConnections[idFState]):
                ...
                for idFragment in composition:
                    if fragmentsRegions[idFragment] == "active":
                        ...
                        speciesFState.append(
                            Species(mol.get_formula(), gas_energy=getGasFormationEnergy(mol, amsResults["Energy"] / eV))
                        )

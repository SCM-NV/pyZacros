    ...
    def __init__(self, data=[], fileName=None, gas_species=None, surface_species=None):
        ...
        if fileName is not None:
            if gas_species is not None and surface_species is not None:
                ...
                raise Exception(
                    "Error: Parameters gas_species and surface_species are requiered to load the Mechanism from a zacros input file"
                )
        ...
        while nline < len(file_content):
            ...
            if tokens[0].lower() == "reversible_step" or tokens[0].lower() == "step":
                ...
                while nline < len(file_content):
                    ...
                    cases = {
                        "gas_reacs_prods": lambda sv: parameters.setdefault(
                            "gas_reacs_prods", process_gas_reacs_prods(sv)
                        ),
                            ...
                            if tokens[0] + tokens[1] + tokens[2] != "&&&":
                                ...
                                if loc_id is None:
                                    raise Exception(
                                        "Error: Species "
                                        + species_name
                                        + " not found! See mechanism initial: "
                                        + parameters["label"]
                                    )
                ...
                if "gas_reacs_prods" in parameters:
                    del parameters["gas_reacs_prods"]

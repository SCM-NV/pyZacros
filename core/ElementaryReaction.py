...
class ElementaryReaction:
    ...
    *   ``label`` -- If None, a unique label is generated based on the label of its initial and final states.
    ...
    def __init__(
        ...
        if (type(initial) != SpeciesList and type(initial) != list) or (
            type(final) != SpeciesList and type(final) != list
        ):
        ...
        if type(initial) == list:
            self.initial = SpeciesList(initial)
        ...
        check_mass = True
        if (
            abs(self.initial.mass(self.initial_entity_number)) < 1e-6
            or abs(self.final.mass(self.final_entity_number)) < 1e-6
        ):
            check_mass = False

        if (
            abs(self.initial.mass(self.initial_entity_number) - self.final.mass(self.final_entity_number)) > 1e-6
            and check_mass
        ):
            msg = "\n### ERROR ### ElementaryReaction.__init__.\n"
            ...
            msg += (
                "              initial:mass("
                + str([sp.symbol for sp in self.initial])
                + ")="
                + str(self.initial.mass(self.initial_entity_number))
            )
            msg += (
                ", final:mass("
                + str([sp.symbol for sp in self.final])
                + ")="
                + str(self.final.mass(self.final_entity_number))
                + "\n"
            )
    ...
    def __updateLabel(self):
        ...
        initialLabel = ElementaryReaction.__getSpeciesListFullName(
            self.__initial_adsorbed, self.initial_entity_number, self.site_types
        )

        if len(self.initial.gas_species()) > 0:
            initialLabel += ":" + SpeciesList(self.initial.gas_species()).label()

        finalLabel = ElementaryReaction.__getSpeciesListFullName(
            self.__final_adsorbed, self.final_entity_number, self.site_types
        )
    ...
    def __str__(self):
        ...
        if self.sites != 0:
            ...
            for i in range(self.sites):
                ...
                output += (
                    "    "
                    + str(self.initial_entity_number[i] + 1)
                    + " "
                    + self.initial[i].symbol
                    + " "
                    + str(site_identate[self.initial_entity_number[i]] + 1)
                    + "\n"
                )
            ...
            for i in range(self.sites):
                ...
                output += (
                    "    "
                    + str(self.final_entity_number[i] + 1)
                    + " "
                    + self.final[i].symbol
                    + " "
                    + str(site_identate[self.final_entity_number[i]] + 1)
                    + "\n"
                )

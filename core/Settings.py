...
class Settings(scm.plams.Settings):
    ...
    def __str__(self):
        ...
        if "random_seed" in self:
            output += "random_seed     " + "%10s" % self.get("random_seed") + "\n"
        ...
        for option in ["snapshots", "process_statistics", "species_numbers", "energetics_lists", "process_lists"]:
            if option in self:
                ...
                if pair[0] in ["event", "elemevent", "time", "logtime", "realtime"]:
                    ...
                    elif len(pair) == 3 and pair[2] in ["on", "off"]:
                    ...
                    elif len(pair) == 4 and pair[3] in ["on", "off"]:
        ...
        if "on_sites_seeding_report" in self:
            output += "on_sites_seeding_report         " + str(self.get(("on_sites_seeding_report"))) + "\n"
        if "event_report" in self:
            output += "event_report         " + str(self.get(("event_report"))) + "\n"

        if "max_steps" in self:
            output += "max_steps         " + str(self.get(("max_steps"))) + "\n"
        if "max_time" in self:
            output += "max_time          " + str(self.get(("max_time"))) + "\n"
        if "wall_time" in self:
            output += "wall_time         " + str(self.get(("wall_time"))) + "\n"

        if "override_array_bounds" in self:
            output += "override_array_bounds         " + str(self.get(("override_array_bounds"))) + "\n"

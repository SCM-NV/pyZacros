"""Functions to retrieve ab-initio data from .rkf files."""
from scm.plams.interfaces.adfsuite.ams import AMSResults, Molecule
...
def get_LatticeAndMechanism(results: AMSResults) -> (Lattice,
                                                     List[ElementaryReaction]):
    """
    Retrive from a .rfk file the surface lattice and mechanism.

    :param results: Entry PLAMS results.

    :return tuple: The slab lattice and the mechanism made of
    a List[ElementaryReaction].
    """
    # Initialize the mechanism list:
    mechanism = []
    ...
    latticeVectors = [[latticeVectors[3*i+j]/angs
                      for j in range(nLatticeVectors)]
                      for i in range(nLatticeVectors)]
    regions = results.readrkf("Molecule", "EngineAtomicInfo").split()
    nStates = results.readrkf("EnergyLandscape", "nStates")
    fileNames = results.readrkf("EnergyLandscape",
                                "fileNames").replace(".rkf", "").split()
    ...
    latticeDisplacements = results.readrkf("BindingSites",
                                           "LatticeDisplacements")
    latticeDisplacements = [[latticeDisplacements[3*i+j] for j in range(3)]
    ...
    # It is not necessary for fix the parentAtomsRaw, because PLAMS
    ...
    for i, fileName in enumerate(fileNames):
        ...
        # vibrations = results.read_rkf_section("Vibrations",
        #                                       file=fileNames[i])
        # print("Frequencies = ", vibrations["Frequencies[cm-1]"])

    # parentStates and parentAtoms elements are actually vectors for each
    # binding site.
    ...
    # The binding sites for a TS are the union of the ones from its reactants
    # and products.
    #
    # TS ----> R --> bsR1, bsR2, ...
    #     |
        ...
        for i, idState in enumerate(parentStates[idSite]):
            ...
            for sMol in adsorbateMols:
                for atom in sMol:
                    if(all([abs(
                           atom.coords[i] -
                           state2Molecule[idState][attachedAtom].coords[i])
                            < 1e-6])):
    ...
    for idState in range(nStates):
        if(isTS[idState]):
            ...
            attachedMolecule[idState] = {**attachedMolecule[idReactant],
                                         **attachedMolecule[idProduct]}

    # Each TS defines an ElementaryReaction and at the same time it
    # defines two Clusters from reactants and products.
    ...
    for idState in range(nStates):
        if(isTS[idState]):
            ...
            clusterReactant = Cluster(site_types=[labels[j]
                                                  for j in connectedSites],
            ...
            clusterProduct = Cluster(site_types=[labels[j]
                                                 for j in connectedSites],
                                     neighboring=neighboring,
                                     species=[Species(f+"*", 1)
                                              for f in species],
            ...
            reaction = ElementaryReaction(site_types=list(labels[j]
                                                          for j in
                                                          connectedSites),
                                          neighboring=neighboring,
                                          initial=clusterReactant.species,
                                          final=clusterProduct.species,
                                          reversible=True,
                                          pre_expon=1e+13,
                                          pe_ratio=0.676,
    ...
    mechanism = get_unique_reactions(mechanism)
    ...
    lattice = Lattice(lattice_type="periodic_cell",

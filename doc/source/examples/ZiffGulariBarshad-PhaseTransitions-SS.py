...
previousJob = len(x_CO)*[ None ]
jobsList = len(x_CO)*[ None ]
isConverged = len(x_CO)*[ False ]
tmax = len(x_CO)*[ 0.0 ]
TOF_CO2 = len(x_CO)*[ None ]
prevTOF_CO2 = len(x_CO)*[ None ]
...
for iter in range(max_iter):
   ...
   # Loop over the x_CO array to run the jobs
   ...
   # Loop over the x_CO array to collect the results and check convergence
   for i,x in enumerate(x_CO):
      if( isConverged[i] ): continue
   ...
   print("----------------------------------------------------------------------------------")
   print("%4s"%"cond", "%8s"%"x_CO", "%18s"%"TOFs[CO2]", "%18s"%"prevTOFs[CO2]", "%10s"%"tmax(s)", "%10s"%"Converged?")
   print("----------------------------------------------------------------------------------")

   for i,x in enumerate(x_CO):
      if( prevTOF_CO2[i] is not None ):
         print("%4d"%i, "%8.2f"%x, "%18.6f"%TOF_CO2[i], "%18.6f"%prevTOF_CO2[i], "%10.1f"%tmax[i], "%10s"%isConverged[i])
      else:
         print("%4d"%i, "%8.2f"%x, "%18.6f"%TOF_CO2[i], "%18s"%"--", "%10.1f"%tmax[i], "%10s"%isConverged[i])
...
for i,x in enumerate(x_CO):
   ...
   print("Entonces = ", jobsList[i].results.lattice_states(last=n_lattice_states))

   for lattice_state in jobsList[i].results.lattice_states(last=n_lattice_states):
      if( lattice_state is not None ):
         print("Hola3", str(lattice_state)[:10])
         fractions = lattice_state.coverage_fractions()
         acf["O*"] += fractions["O*"]/n_lattice_states
         acf["CO*"] += fractions["CO*"]/n_lattice_states
      else:
         print("Hola3", lattice_state)
...
for i,x in enumerate(x_CO):
    print("%8.2f"%x, "%10.6f"%cf_O[i], "%10.6f"%cf_CO[i], "%10.6f"%TOF_CO2[i])
...
#jobsList[33].results.last_lattice_state().plot()
#jobsList[34].results.last_lattice_state().plot()
